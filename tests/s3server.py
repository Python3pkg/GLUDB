...
# Taken verbatim from demos/s3server/s3server.py in GitHub Repo
# tornadoweb/tornado to be used here in gludb testing. Our only
# additions were:
#  1. Adding the main logic at the end of the file (which includes pre-creating
#     a bucket we use for testing)
#  2. Adding support for HEAD bucket per
#     http://docs.aws.amazon.com/AmazonS3/latest/API/RESTBucketHEAD.html
#  3. Adding correct ETag support
#  4. Sending a custom x-amz-server-side-encryption-customer-algorithm back
#     so that recent boto implementations don't choke on ETag's
...
def main():
    ...
    start(8888, root_directory=S3_DIR)
